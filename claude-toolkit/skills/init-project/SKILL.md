...
## 계획 자동 저장 규칙
- 사용자와 계획을 수립하면 즉시 `notes/{plan_name}_plan.md`로 저장할 것 (묻지 말고 자동 저장)
- 계획 파일은 상태(Phase 진행 중/완료), 목적, 단계별 작업, 산출물, 완료 조건 포함
- 수정/진전이 있으면 같은 파일을 업데이트 (새 파일 만들지 말 것)
- 아래 "현재 진행 중 계획" 섹션에 링크 추가 — 파일 생성/완료 시 같이 갱신

## 현재 진행 중 계획
<!-- 계획 파일을 만들면 여기에 `- [plan_name](notes/plan_name_plan.md) — 한 줄 설명` 형식으로 추가. 완료되면 제거. -->
